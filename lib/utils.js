...
async function getApiKey(profileName = null) {
  ...
  // Fall back to legacy apiKey field
  if (config.apiKey) {
  ...
  if (apiKey) {
    // Save to legacy apiKey field for backward compatibility
...
function generateSlugFromUrl(url) {
  try {
    ...
    // Remove protocol and common prefixes, replace dots with empty string
    return hostname.replace(/^www\./, '').replace(/\./g, '').toLowerCase();
  } catch (error) {
    return null;
  }
}
...
function addProfile(slug, url, apiKey, setAsDefault = false) {
  const config = readConfig();
